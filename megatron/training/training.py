...
def wrap_model_chunks_with_ddp(
    ...
    *,
    ...
    For ``use_layer_wise_distributed_optimizer=True``: tags params (so BufferKey
    separates layerwise-managed buffers from non-layerwise ones), forces
    ...
    Layouts are only computed for ``DP is DistributedDataParallel``; FSDP
    variants don't accept ``full_param_layout``.
    ...
    Args:
        ...
        optimizer_name: Name of the emerging optimizer (e.g. ``'muon'``).
            Required when ``use_layer_wise_distributed_optimizer=True``.
    ...
    if DP is DistributedDataParallel:
        if use_layer_wise_distributed_optimizer:
            LayerWiseDistributedOptimizer.tag_params(model_chunks, optimizer_name)
    ...
    for chunk, layout, disable_bucketing in zip(
        ...
        chunk_kwargs = {}
        if pg_collection is not None and DP is not DistributedDataParallel:
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True, config=None, pg_collection=None):
    ...
    # Translate legacy "dist_*" optimizer names (e.g. "dist_muon" → "muon") early
    # so the layerwise wiring below (use_distributed_optimizer, tag_params, layout)
    # runs on the canonical name. The deprecation warning still fires later inside
    # _get_megatron_emerging_optimizer.
    if getattr(args, 'optimizer', None) and args.optimizer.startswith('dist_'):
        args.optimizer = args.optimizer[len('dist_'):]
