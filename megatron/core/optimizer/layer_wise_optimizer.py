...
class LayerWiseDistributedOptimizer(ChainedOptimizer):
    ...
    def tag_params(model_chunks, eopt_name, config_overrides=None):
        ...
        # Translate legacy "dist_*" optimizer names (e.g. "dist_muon" → "muon")
        # so callers that pass the deprecated alias still get correct tagging.
        if eopt_name.startswith('dist_'):
            eopt_name = eopt_name[len('dist_') :]
